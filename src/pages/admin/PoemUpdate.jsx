...
import { getPoem, updatePoem } from "../../apis";
...
const handleGetPoem = async () => {
  const response = await getPoem(auth?.token, pathname?.id);
  setPoemTitle(response?.data?.poem?.title)
  setPoem(response?.data?.poem?.poem)
  setPoemMedia(response?.data?.poem?.videoUrl)
}
...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    setLoading(true);
    e.preventDefault();
    const response = await updatePoem(auth?.token, pathname?.id, poemTitle, poem, poemMedia);
