...
import { getSpeaking, updateSpeakingEngagement } from "../../apis";
...
const SpeakingUpdate = () => {
  ...
  const handleGetSpeakingEngagement = async () => {
    const response = await getSpeaking(auth?.token, pathname?.id);
    setEvent(response?.data?.speaking?.event)
    setLocation(response?.data?.speaking?.location)
    setDate(response?.data?.speaking?.date)
    setSpeakingMedia(response?.data?.speaking?.media)
  }
  
  useEffect(() => {
    handleGetSpeakingEngagement()
  }, [])
  ...
  const handleSubmit = async (e) => {
    setLoading(true);
    e.preventDefault();
    const response = await updateSpeakingEngagement(auth?.token, pathname?.id, event, location, date, speakingMedia);
  ...
  return (
    <div className="adminContainer reg">
      ...
      <div className="adminContainer__main">
        ...
        <h1>Update Speaking Engagement</h1>
        <form className="form" onSubmit={handleSubmit}>
          ...
          <button className="largeButton" type="submit" disabled={loading || uplaodLoading}>
            {loading ? "Loading..." : "Update Speaking Engagement"}
          </button>
        </form>
      </div>
    </div>
